...
export default function MarsSurvivalGame() {
  const [view, setView] = useState<'login' | 'story' | 'game' | 'results' | 'admin' | 'leaderboard' | 'user-detail'>('login');
  const [username, setUsername] = useState('');
  const [team, setTeam] = useState('');
  const [items, setItems] = useState<SurvivalItem[]>([]);
  const [allResults, setAllResults] = useState<UserResult[]>([]);
  const [selectedUserDetail, setSelectedUserDetail] = useState<UserResult | null>(null);
  
  // Состояния для админки
  const [story, setStory] = useState(INITIAL_STORY);
  const [teamsList, setTeamsList] = useState(['Alfa Centauri', 'Marineris Rangers', 'Olympus Mons']);
  ...
  const getScoreMessage = (s: number) => {
    ...
    if (s <= 50) return "SUFFICIENTE. Probabilmente finirai l'energia a metà strada.";
    ...
    <CRTWrapper>
      ...
      <div className="flex flex-col gap-6 max-w-sm mx-auto py-12">
        <div className="space-y-2">
          <label className="text-xs uppercase">Identificativo Operatore:</label>
          <input 
    ...
    <CRTWrapper>
      <div className="flex justify-between items-end mb-6">
        <div className="text-xs">
          OPERATORE: {username}<br/>TEAM: {team}
        </div>
        <h2 className="text-xl font-bold uppercase tracking-widest">Configurazione Inventario</h2>
      </div>
      ...
      <Reorder.Group axis="y" values={items} onReorder={setItems} className="space-y-2">
        {items.map((item, index) => (
          ...
          >
            <span className="text-2xl font-black w-10 text-[#00ff41]/40 group-hover:text-[#00ff41]">{index + 1}</span>
            <div className="w-12 h-12 bg-[#222] border border-[#00ff41]/20 flex items-center justify-center text-[10px] text-center">
              IMG
            </div>
    ...
    return (
      <CRTWrapper>
        ...
        <div className="text-center mb-8">
          ...
          <div className="text-sm uppercase tracking-[0.3em] mb-4">Punti di Deviazione</div>
        ...
        <div className="grid gap-4 mb-8 h-64 overflow-y-auto border border-[#00ff41]/30 p-4 bg-black/50">
          {INITIAL_ITEMS.sort((a,b) => a.idealPosition - b.idealPosition).map(item => (
            <div key={item.id} className="text-xs border-b border-[#00ff41]/20 pb-2">
              <span className="text-[#00ff41] font-bold">{item.idealPosition}. {item.name}</span>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <button onClick={() => setView('leaderboard')} className="flex-1 border-2 border-[#00ff41] py-3 hover:bg-[#00ff41] hover:text-black uppercase font-bold">Classifica Team</button>
          <button onClick={() => window.location.reload()} className="flex-1 border-2 border-[#00ff41]/50 py-3 hover:text-white uppercase font-bold text-xs flex items-center justify-center gap-2">
  ...
  if (view === 'leaderboard') return (
    <CRTWrapper>
      <div className="flex justify-between items-center mb-6 border-b-2 border-[#00ff41] pb-2">
        <button onClick={() => setView('results')} className="text-xs flex items-center gap-1 hover:underline"><ArrowLeft size={14}/> Indietro</button>
        <h2 className="text-xl font-bold uppercase">Status Coloni</h2>
        <button onClick={() => setAllResults(JSON.parse(localStorage.getItem('mars_results') || '[]'))} className="p-1 hover:rotate-180 transition-transform"><RefreshCcw size={18}/></button>
      </div>
      ...
      <div className="space-y-2">
        ...
        {allResults.sort((a,b) => a.score - b.score).map((res, i) => (
          <div key={i} className="grid grid-cols-4 items-center bg-[#111] p-4 border border-[#00ff41]/20 hover:border-[#00ff41]">
            ...
            <button 
              onClick={() => { setSelectedUserDetail(res); setView('user-detail'); }}
  ...
  if (view === 'user-detail' && selectedUserDetail) return (
    <CRTWrapper>
      <div className="mb-6">
        <button onClick={() => setView('leaderboard')} className="text-xs flex items-center gap-1 hover:underline mb-4"><ArrowLeft size={14}/> Torna alla Classifica</button>
        <h2 className="text-2xl font-bold uppercase tracking-tighter">Analisi: {selectedUserDetail.username}</h2>
        <div className="text-sm opacity-70 italic">Team: {selectedUserDetail.team} | Score: {selectedUserDetail.score}</div>
      </div>
      ...
      <div className="space-y-1 text-xs">
        {selectedUserDetail.selections.map((itemId, idx) => {
          ...
          return (
            <div key={itemId} className="flex justify-between p-2 border-b border-[#00ff41]/10 bg-black/30">
              <span>{idx + 1}. {item?.name}</span>
              <span className={diff === 0 ? "text-green-400" : "text-amber-500"}>NASA: {item?.idealPosition} (Δ{diff})</span>
            </div>
          );
        })}
      </div>
  ...
  if (view === 'admin') return (
    <CRTWrapper>
      <div className="flex justify-between items-center mb-8 border-b-4 border-[#00ff41] pb-2">
        <h2 className="text-2xl font-black italic uppercase bg-[#00ff41] text-black px-2">Admin Terminal</h2>
        <button onClick={() => setView('login')} className="text-xs underline">LOGOUT</button>
      </div>

      <div className="grid md:grid-cols-2 gap-8">
        {/* Teams Management */}
        <div className="space-y-4 border-2 border-[#00ff41]/30 p-4">
          <h3 className="font-bold uppercase flex items-center gap-2"><Users size={18}/> Gestione Team</h3>
          <div className="space-y-2">
            {teamsList.map(t => (
              <div key={t} className="flex justify-between items-center bg-[#111] p-2 text-sm">
                <span>{t}</span>
                <button onClick={() => setTeamsList(teamsList.filter(i => i !== t))}><Trash2 size={14} className="text-red-500"/></button>
              </div>
            ))}
          </div>
          <button 
        ...
        {/* Story Management */}
        <div className="space-y-4 border-2 border-[#00ff41]/30 p-4">
          <h3 className="font-bold uppercase flex items-center gap-2"><Edit size={18}/> Editor Scenario</h3>
          <div className="space-y-2">
            <label className="text-[10px]">Titolo:</label>
            <input className="w-full bg-black border border-[#00ff41] p-1 text-sm" value={story.title} onChange={e => setStory({...story, title: e.target.value})}/>
            <label className="text-[10px]">Plot:</label>
            <textarea className="w-full bg-black border border-[#00ff41] p-1 text-xs h-24" value={story.plot} onChange={e => setStory({...story, plot: e.target.value})}/>
          </div>
        </div>
      </div>

      <div className="mt-8">
        <h3 className="font-bold uppercase mb-4 flex items-center gap-2"><Save size={18}/> Database Risultati</h3>
            ...
            <tbody>
              {allResults.map((r, i) => (
                <tr key={i} className="border-b border-[#00ff41]/20">
                  ...
                  <td className="p-1">
                    <button onClick={() => {
                      const updated = allResults.filter((_, idx) => idx !== i);
                      setAllResults(updated);
                      localStorage.setItem('mars_results', JSON.stringify(updated));
      ...
      <button onClick={() => setView('leaderboard')} className="w-full mt-8 border-2 border-[#00ff41] py-2 hover:bg-[#00ff41] hover:text-black uppercase font-bold">
